...
export async function authenticateUser(
  ...
  let detectedRole: 'admin' | 'teacher' | 'student' | null = null;
  ...
  if (!user || !detectedRole) return null;
  ...
  return { token, user: { id: user.id, name: user.name, email: user.email, role: detectedRole } };
}
